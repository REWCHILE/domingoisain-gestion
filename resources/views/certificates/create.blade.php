...
@section('title', 'Emitir Certificado / Cotización')
...
@php
    ...
    $initialDocType = old('document_type', 'certificado');
    $initialRegion = old('client_region', 'Región Metropolitana');
    $initialComuna = old('client_comuna', 'La Florida');
    $initialWorkDetails = old('work_details', '');
@endphp

<!-- Form Global Alpine Component Script Loaded FIRST -->
<script>
window.certificateData = function() {
    ...
    <div class="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                ...
                <span>Nuevo Certificado de Servicio / Cotización</span>
            </h1>
    ...
    <form action="{{ route('certificates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200/90 shadow-xs space-y-4">
            ...
            <div>
                ...
                <input type="text" id="client_address" name="client_address" value="{{ old('client_address') }}"
                       class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-slate-800 text-sm font-medium focus:bg-white focus:border-sky-500 shadow-xs">
            </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200/90 shadow-xs space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div>
                    ...
                    <input type="text" id="gasfiter_name" name="gasfiter_name" value="{{ old('gasfiter_name', 'Domingo Isain Plaza Caamaño') }}"
                ...
                <div>
                    ...
                    <input type="text" id="gasfiter_rut" name="gasfiter_rut" value="{{ old('gasfiter_rut', '12738961-6') }}"
                           class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-slate-800 text-xs font-semibold focus:bg-white focus:border-sky-500 shadow-xs">
                </div>
                <div>
                    ...
                    <input type="text" id="gasfiter_sec_class" name="gasfiter_sec_class" value="{{ old('gasfiter_sec_class', 'Gasfiter Certificado Autorizado SEC') }}"
                           class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl text-slate-800 text-xs font-semibold focus:bg-white focus:border-sky-500 shadow-xs">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-4 border-t border-slate-200">
            ...
            <button type="submit" class="px-8 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-black text-sm rounded-xl shadow-lg shadow-emerald-500/25 hover:-translate-y-0.5 transition-all flex items-center gap-2 cursor-pointer">
                <i data-lucide="check-circle" class="w-5 h-5"></i> <span>Emitir Certificado N° {{ $nextNumber }}</span>
            </button>
        </div>
